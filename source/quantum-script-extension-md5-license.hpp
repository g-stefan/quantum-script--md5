//
// Quantum Script Extension MD5
//
// Copyright (c) 2020-2022 Grigore Stefan <g_stefan@yahoo.com>
// Created by Grigore Stefan <g_stefan@yahoo.com>
//
// MIT License (MIT) <http://opensource.org/licenses/MIT>
//

#ifndef QUANTUM_SCRIPT_EXTENSION_MD5_LICENSE_HPP
#define QUANTUM_SCRIPT_EXTENSION_MD5_LICENSE_HPP

#ifndef QUANTUM_SCRIPT_EXTENSION_MD5__EXPORT_HPP
#	include "quantum-script-extension-md5--export.hpp"
#endif

namespace Quantum {
	namespace Script {
		namespace Extension {
			namespace MD5 {
				namespace License {
					QUANTUM_SCRIPT_EXTENSION_MD5_EXPORT const char *content();
					QUANTUM_SCRIPT_EXTENSION_MD5_EXPORT const char *shortContent();
				};
			};
		};
	};
};

#endif
